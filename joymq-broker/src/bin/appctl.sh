...
start ()
{
    ...
    for e in $(seq 90); do
        echo "check http://127.0.0.1:${STATUS_PORT}/status.broker ${e}"
        sleep 1
        status_code=`/usr/bin/curl -L -o /dev/null --connect-timeout 1 -s -w %{http_code}  "http://127.0.0.1:${STATUS_PORT}/status.broker"`
        if [ ${status_code} == 200 ];then
            echo "check http://127.0.0.1:${STATUS_PORT}/status.broker success"
            return 0
        fi
    done
    echo "check http://127.0.0.1:${STATUS_PORT}/status.broker fail"
    return 1 # status.broker check failed
}
...
online()
{
    touch -m $APP_HOME/status.broker
}

offline()
{
    rm -f $APP_HOME/status.broker
}
